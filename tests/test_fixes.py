...
for config_name in ["phase3_config_a", "phase3_config_b", "phase3_config_c"]:
    ...
    print(
        f"  {config_name}: dinov2.freeze_all={cfg.train.dinov2.freeze_all}, unfrozen_blocks={cfg.train.dinov2.unfrozen_blocks} ✓"
    )
...
assert (
    dm.get_current_n_sources() == 2
), f"Initial n_sources should be 2, got {dm.get_current_n_sources()}"
dm.update_curriculum(0)
assert (
    dm.get_current_n_sources() == 2
), f"Step 0 n_sources should be 2, got {dm.get_current_n_sources()}"
dm.update_curriculum(10000)
assert (
    dm.get_current_n_sources() == 2
), f"Step 10000 n_sources should be 2, got {dm.get_current_n_sources()}"
dm.update_curriculum(25000)
assert (
    dm.get_current_n_sources() == 3
), f"Step 25000 n_sources should be 3, got {dm.get_current_n_sources()}"
dm.update_curriculum(45000)
assert (
    dm.get_current_n_sources() == 4
), f"Step 45000 n_sources should be 4, got {dm.get_current_n_sources()}"
dm.update_curriculum(100000)
assert (
    dm.get_current_n_sources() == 4
), f"Step 100000 n_sources should be 4, got {dm.get_current_n_sources()}"
